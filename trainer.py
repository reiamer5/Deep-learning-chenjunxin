...
from mdoel.convnext import convnext_base_one, convnext_small_one, convnext_tiny_one, convnext_large_one
from utils import timeSince, create_dir
from networks import classifer
...
parser.add_argument('--gpu', default=2, type=int, help='')
...
class Classifier_Trainer(object):
    ...
    def set_model(self, model_type="convnext_base"):
        ...
        if model_type == "convnext_base":
            ...
            model = convnext_large_one(pretrained=True, in_22k=True, num_classes=2)
        else:
    ...
    def start(self, model_path=None):
        ...
        for epoch in range(self.epoch):
            print("\n----------start to train epoch:%s----------" % (epoch + 1))
            self.train(epoch)
            print("----------start to value epoch:%s----------" % (epoch + 1))
            acc = self.val()
            # 学习率策略
        ...
        print("所花费的时间:%s" % (timeSince(self.startTime)))  # 输出训练时间
    ...
    def test(self, model_path=None):
        ...
        submit_data.to_csv("submission_convnext_alexnet_wu.csv", index=False)
        print("test完成!")


trainer = Classifier_Trainer()
#trainer.start()
path = "checkpoints/alexnet/0.9983.pth"
